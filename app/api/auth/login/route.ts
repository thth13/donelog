...
export async function POST(request: Request) {
  ...
  if (!/^done_[a-f0-9]{64}$/.test(secret)) return NextResponse.json({ error: "Invalid secret key. Check the key and try again." }, { status: 401 });
  try {
    await connectDB();
    const user = await User.findOne({ secretHash: hashSecret(secret) });
    if (!user) return NextResponse.json({ error: "Invalid secret key. Check the key and try again." }, { status: 401 });
