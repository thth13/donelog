...
export function AccountMenu() {
  ...
  return <div className="account-menu"><button type="button" className="account-key" title="Copy secret key" aria-label={preview ? `Copy secret key ${preview}` : "Load and copy secret key"} disabled={busy || loadingKey} aria-busy={busy || loadingKey} onClick={() => void copyKey()}>{preview || (loadingKey ? "Loading key…" : "Key unavailable")}</button><button type="button" disabled={busy} aria-busy={busy} onClick={() => void signOut()}>Sign out</button><span className="account-error" role="alert">{error}</span><span className="account-copy-status" role="status">{copied ? "Key copied" : ""}</span></div>;
}
