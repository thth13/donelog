...
`DailyReportDialog` owns the daily report preview and sharing disclosure. Statistics opens today's report from its heading or any recorded day from history. Reports use the same browser-local day boundaries as statistics, include merged local/server entries, exclude deleted tasks, and label partial history explicitly. The visual is a journal sheet with the existing Manrope, cream/green tokens, completion checks, chronological tasks, and times. The native dialog follows existing modal focus containment, Escape, close, scroll locking, and focus restoration. Sharing includes the displayed day's text only: clipboard with inline recovery, WhatsApp/email compose links, and feature-detected native sharing. Long reports use clipboard/native sharing to avoid truncating compose links. Nothing is published automatically. Runtime styles remain in `app/globals.css`; no new dependencies or public report endpoint are introduced.
